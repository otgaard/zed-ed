import{Ct as ee,dt as e,it as t}from"./project.svelte-DKj3yU6X.js";import{f as te}from"./includeClosure-CBOfwEfO.js";import{o as ne}from"./bvhConfig-vzOajthA.js";import{a as re,c as ie,i as n,n as r,o as i,r as a,s as o,t as s}from"./raymarchGlsl-CZdfVUqY.js";var c=`#ifndef ZED_SS_COMMON_INCLUDED
...
// Soft area shadow: average the visibility from this CASTER's K origin points sampled
// across its rect. Every area caster is soft — its per-origin VPs live in a contiguous
// slice of the shared uAreaVP pool (base = uShadowAreaInfo[slot].x, count = .y) with
// matching consecutive depth-array layers from the slot's baseLayer. The spread of
// occlusion across origins IS the penumbra, so its width tracks the rect's size — true
// area soft shadows, not a fixed blur. More samples ⇒ a smoother penumbra.
float zShadowArea(int slot, vec3 worldPos, vec3 N) {
  float bias = uShadowParams[slot].x;
  float normalBias = uShadowParams[slot].y;
  vec3 p = worldPos + N * normalBias;
  float baseLayer = uShadowMapInfo[slot].x;
  int base = int(uShadowAreaInfo[slot].x);             // this caster's uAreaVP slice
  int count = int(uShadowAreaInfo[slot].y);            // its granted sample count
  float vis = 0.0;
  for (int k = 0; k < ZED_MAX_AREA_SHADOW_SAMPLES; k++) {
    if (k >= count) break;                             // only this caster's samples
    vec4 clip = uAreaVP[base + k] * vec4(p, 1.0);
...
// A lane read, not a slot scan: the pack writes slot + 1 into uLightBitangent[i].w
// (0 = casts nothing). The old linear scan over uShadowMapInfo was 16 casters ×
// 16 lights = up to 256 UBO reads per fragment inside the model's punctual loop —
// and <lights> is already a hard requirement of this include (zShadowCube reads
// uLightPos), so the lane costs no new coupling.
int shadowSlotForLight(int lightIndex) {
  return int(uLightBitangent[lightIndex].w) - 1;
}
